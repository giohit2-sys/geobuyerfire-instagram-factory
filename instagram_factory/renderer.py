...
def _font(size: int = FONT_SIZE) -> ImageFont.FreeTypeFont:
    face = ImageFont.truetype(str(FONT), size=size)
...
def render_slide(
    ...
    # Cover variant #1: the hook is deliberately larger while staying inside
    # the central 3:4 grid-safe area. Body cards keep the established size.
    face = _font(COVER_FONT_SIZE if index == 1 else FONT_SIZE)
